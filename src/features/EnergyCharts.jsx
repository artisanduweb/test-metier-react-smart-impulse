...
const aggregateDataByWeek = (data) => {
  ...
  data.forEach((series) => {
    ...
    aggregatedData[series.label] = Object.entries(weeklyData).map(
      ([timestamp, value]) => [parseInt(timestamp), value]
    );
  });
  ...
  useEffect(() => {
    if (data.length) {
      const maxDate = new Date(
        Math.max(
          ...data.map((series) => series.data[series.data.length - 1][0])
        )
      );
      const minDate = addDays(maxDate, -15);
      setDate({
        from: toZonedTime(minDate, timezone),
        to: toZonedTime(maxDate, timezone),
      });
    }
  }, [data, timezone]);
    ...
    (data, fromDate, toDate) => {
      return data.map((series) => ({
        ...
        data: series.data
          ...
          .map((point) => [
            point[0],
            YValue === "currency" ? point[1] * EURO_PER_MWH : point[1],
  ...
  const getOptions = useCallback(() => {
    ...
    const filteredData =
      date && date.from && date.to
        ? filterDataByDate(data, date.from, date.to)
        : data;
    const aggregatedData =
      view === "week" ? aggregateDataByWeek(filteredData) : filteredData;
          ...
          <CardContent>
            <RadioGroup onValueChange={setView}>
              <div className="flex gap-2">
                ...
                <Label htmlFor="r1">Jour</Label>
              </div>
              <div className="flex gap-2">
                <RadioGroupItem value="week" id="week">
                  Semaine
                </RadioGroupItem>
                <Label htmlFor="r2">Semaine</Label>
              </div>
              <div className="flex gap-2">
                <RadioGroupItem value="month" id="month">
                  Mois
                </RadioGroupItem>
                <Label htmlFor="r3">Mois</Label>
              </div>
            </RadioGroup>
          </CardContent>
        </Card>
